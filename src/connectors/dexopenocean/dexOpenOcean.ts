...
export class DexOpenOcean implements Uniswapish {
  private static _instances: { [name: string]: DexOpenOcean };
  ...
  private constructor(chain: string, network: string) {
    ...
    const config = DexOpenOceanConfig.config;
  ...
  public static getInstance(chain: string, network: string): DexOpenOcean {
    if (DexOpenOcean._instances === undefined) {
      DexOpenOcean._instances = {};
    }
    if (!(chain + network in DexOpenOcean._instances)) {
      DexOpenOcean._instances[chain + network] = new DexOpenOcean(
        chain,
        network
      );
    }

    return DexOpenOcean._instances[chain + network];
  }
  ...
  getSlippageNumberage(): number {
    const allowedSlippage = DexOpenOceanConfig.config.allowedSlippage;
