...
export abstract class Router {
  ...
  public static swapCallParameters(
    ...
    if (pathlen === 2) {
      methodName = 'swap';
      let singleSwap = [path[0], path[1], amountIn];
      let fundManagement = [wallet.address, wallet.address];
      // TODO: Update
      let limit = ZERO_HEX;
  ...
  public static swapCallParametersForCP(
    trade: Trade,
    options: TradeOptions | TradeOptionsDeadline
  ): SwapParameters {
    const etherIn = trade.inputAmount.currency === ETHER;
    ...
    if (pathlen === 2) {
      methodName = 'safeSwap';
      args = [[path[0], path[1], amountIn], ZERO_HEX, deadline];
      value = ZERO_HEX;
    } else if (pathlen > 2) {
