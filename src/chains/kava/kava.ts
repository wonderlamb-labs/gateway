...
import { getEthereumConfig as getKavaConfig } from '../ethereum/ethereum.config';
import { Provider } from '@ethersproject/abstract-provider';
import { Ethereumish } from '../../services/common-interfaces';
import { GamutConfig } from '../../connectors/gamut/gamut.config';
import { ConfigManagerV2 } from '../../services/config-manager-v2';

export class Kava extends EthereumBase implements Ethereumish {
  private static _instances: { [name: string]: Kava };
  ...
  private constructor(network: string) {
    const config = getKavaConfig('kava', network);
    super(
      'kava',
  ...
  public static getInstance(network: string): Kava {
    if (Kava._instances === undefined) {
      Kava._instances = {};
    }
    if (!(network in Kava._instances)) {
      Kava._instances[network] = new Kava(network);
    }

    return Kava._instances[network];
  }

  public static getConnectedInstances(): { [name: string]: Kava } {
    return Kava._instances;
  }
  ...
  getSpender(reqSpender: string): string {
    let spender: string;
    if (reqSpender === 'gamut') {
      spender = GamutConfig.config.routerAddress(this._chain);
    } else {
      spender = reqSpender;
    }
    return spender;
